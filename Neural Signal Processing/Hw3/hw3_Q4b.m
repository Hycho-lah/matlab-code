...
for n = 1:91
    for k =1:8 
        ...
        args = [];
        for c = 1:8
            arg = get_arg_ak(mu_array(:,c),cell2mat(sigma_array(c)),test_point);
            args = [args, arg];
        end
        [arg_max,C] = max(args);
        FR_test_class(n,k) = C;
...
function arg_ak = get_arg_ak(mu,sigma,point)
    arg_ak = -(1/2)*transpose(point)*inv(sigma)*point + transpose(mu)*inv(sigma)*point - (1/2)*transpose(mu)*inv(sigma)*mu - (1/2)*log(det(sigma)) + log(1/8); 
end
