...
sigma = 20/60*(calc_Sk(points_1,mu_1) + calc_Sk(points_2,mu_2) + calc_Sk(points_3,mu_3));
...
for x = 1:101
    for y = 1:101
        point = [X(x,y);Y(x,y)];
        arg_C1 = transpose(mu_1)*inv(sigma)*point-(1/2)*transpose(mu_1)*inv(sigma)*mu_1+log(20/60);
        arg_C2 = transpose(mu_2)*inv(sigma)*point-(1/2)*transpose(mu_2)*inv(sigma)*mu_2+log(20/60);
        arg_C3 = transpose(mu_3)*inv(sigma)*point-(1/2)*transpose(mu_3)*inv(sigma)*mu_3+log(20/60);
...
[X,Y] = meshgrid(x1_array,y1_array);
F = mvnpdf([X(:) Y(:)],transpose(mu_1),sigma);
...
[X,Y] = meshgrid(x1_array,y1_array);
F = mvnpdf([X(:) Y(:)],transpose(mu_2),sigma);
F = reshape(F,length(y1_array),length(x1_array));
...
[X,Y] = meshgrid(x1_array,y1_array);
F = mvnpdf([X(:) Y(:)],transpose(mu_3),sigma);
F = reshape(F,length(y1_array),length(x1_array));
